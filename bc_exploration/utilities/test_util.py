...
def test_round_to_increment():
    ...
    assert round_to_increment(22.0, 1.00) == 22
...
def test_compare_maps_with_floodfill():
    ...
    current_map = cv2.imread(
        get_maps_dir() + "/test/vw_partly_explored_test.png", cv2.COLOR_BGR2GRAY
    )
    current_map = Costmap(current_map, 1, np.array([0.0, 0.0]))
