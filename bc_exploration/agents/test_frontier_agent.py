...
def test_extract_frontiers(debug=False):
    ...
    occupancy_map = Costmap(
        data=occupancy_map_data, resolution=0.03, origin=np.array([0, 0])
    )
    ...
    frontier_sizes = np.array(
        [frontier.shape[0] if len(frontier.shape) > 1 else 1 for frontier in frontiers]
    )
    significant_frontiers = [
        frontier for i, frontier in enumerate(frontiers) if frontier_sizes[i] > 70
    ]
...
def test_plan(debug=False):
    ...
    occupancy_map = Costmap(
        data=occupancy_map_data, resolution=0.03, origin=np.array([-6.305, -6.305])
    )
    pose = np.array([0.8, 0, -0.51759265])

    frontier_agent = create_frontier_agent_from_params(
        os.path.join(get_exploration_dir(), "params/params.yaml")
    )
...
def debug_frontier_agent():
    ...
    frontier_agent = create_frontier_agent_from_params(
        get_exploration_dir() + "/params/params.yaml"
    )
    ...
    draw_footprint_path(
        frontier_agent.get_footprint(), path, visualization_map, [0, 255, 0]
    )
