...
def test_custom_footprint(debug=False):
    ...
    footprint = CustomFootprint(footprint_points, 2 * np.pi / 180.0)
    ...
    assert footprint.check_for_collision(
        colliding_state, occupancy_map, debug=debug, use_python=True
    )

    rotated_colliding_state = rc_to_xy(np.array([110, 220, -np.pi / 2]), occupancy_map)
    assert footprint.check_for_collision(
        rotated_colliding_state, occupancy_map, debug=debug
    )
    assert footprint.check_for_collision(
        rotated_colliding_state, occupancy_map, debug=debug, use_python=True
    )
...
def test_footprint_orientation(debug=False):
    ...
    footprint = CustomFootprint(
        footprint_points, 2 * np.pi / 180.0, inflation_scale=1.3
    )
