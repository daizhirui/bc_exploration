...
def test_one_astar(debug=False):
    ...
    occupancy_map = Costmap(occupancy_map_data, 0.03, origin=[0.0, 0.0])
...
def test_multithread_astar(debug=False):
    ...
    astar_fn = partial(
        astar, start=start, occupancy_map=occupancy_map, obstacle_values=obstacle_values
    )
...
def test_oriented_astar(debug=False):
    ...
    footprint = CustomFootprint(
        footprint_points, 2.0 * np.pi / 180.0, inflation_scale=2.0
    )

    angles = get_astar_angles()

    footprint_masks = footprint.get_footprint_masks(
        occupancy_map.resolution, angles=angles
    )
    outline_coords = footprint.get_outline_coords(
        occupancy_map.resolution, angles=angles
    )

    start = rc_to_xy([1956, 137, 0], occupancy_map)
    goal = rc_to_xy([841, 3403, 0.0], occupancy_map)
    ...
    if debug:
        ...
        draw_footprint_path(
            footprint, path, visualization_map, [255, 0, 0], [0, 0, 255]
        )
...
def test_impossible_path(debug=False):
    ...
    start = rc_to_xy([425, 50, 0.0], occupancy_map)
    goal = rc_to_xy([232, 339, 0.0], occupancy_map)
...
def debug_real_case():
    ...
    footprint = CustomFootprint(footprint_points, np.pi / 4.0, inflation_scale=1.0)
    outline_coords = np.array(footprint.get_outline_coords(occupancy_map.resolution))
    angle_inds = np.argmin(
        np.abs(path[:, 2:] - np.expand_dims(c_angles, axis=0)), axis=1
    )
    draw_footprints = outline_coords[angle_inds] + np.expand_dims(path[:, :2], axis=1)
    draw_footprints = draw_footprints.reshape(
        draw_footprints.shape[0] * draw_footprints.shape[1], draw_footprints.shape[2]
    ).astype(int)
